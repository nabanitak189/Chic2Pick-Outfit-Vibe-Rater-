...
#     st.divider()
#     st.header("Options")
#     show_debug = st.checkbox("Show CLIP probabilities (debug)")
#     presentation_mode = st.checkbox("Presentation Mode", value=True)
#     adjust_strength = st.slider("Adjustment strength", 0.0, 1.0, 0.6, 0.05) if presentation_mode else 0.0
...
# # optional debug view
# if show_debug:
#     st.markdown("#### 🔍 CLIP probabilities")
#     st.json(clip_scores)
...
# from utils.analyzers import aggregate_scores
# from utils.ui import score_chip, verdict, section_header
# from utils.clip_utils import analyze_outfit   # uses improved prompts
...
# # -------------------------
# st.markdown("## 📸 Upload or Capture Outfit")
# img_file = st.file_uploader("Choose an image", type=["jpg","jpeg","png"]) if img_choice=="Upload" else st.camera_input("Capture outfit photo")
...
# # Open image safely
# # -------------------------
# # Use PIL for all downstream (avoid pointer/BytesIO read issues)
# image = Image.open(io.BytesIO(img_file.getvalue())).convert("RGB")
# img_np = np.array(image)

# # -------------------------
# # Run CLIP analyzer on the PIL image (important)
# # -------------------------
# clip_scores = analyze_outfit(image)
...
# # Occasion classification
# # -------------------------
# # prefer the model's best label if exposed, else compute from dict
# # Occasion classification
# if "_best_label" in clip_scores:
#     occasion = clip_scores["_best_label"]
# else:
#     # exclude internal keys and ensure values are float
#     clean = {
#         k: float(v) 
...
# from utils.analyzers import aggregate_scores
# from utils.ui import score_chip, verdict, section_header
# from utils.clip_utils import analyze_outfit   
...
# # -------------------------
# # Open image (works for both Upload + Camera)
# # -------------------------
...
# # Run CLIP analyzer
# # -------------------------
# clip_scores = analyze_outfit(img_file)  # pass directly, util handles types
...
# # Occasion classification
# # -------------------------
# occasion = max(clip_scores, key=clip_scores.get)
# st.success(f"👔 Detected occasion: **{occasion.upper()}**")
...
# from PIL import Image
# from utils.analyzers import aggregate_scores
# from utils.clip_utils import analyze_outfit   # your custom model
...
#     # AI vibe analysis
#     file.seek(0)
#     clip_scores = analyze_outfit(file)   # dict: {"casual":0.7, ...}
#     raw_scores = aggregate_scores(img_np)

#     # Occasion classification
#     occasion = max(clip_scores, key=clip_scores.get)

#     # Simple size suggestion
#     fit_score = raw_scores.get("fit_tightness", 50)
#     if fit_score < 30:
#         suggested_size = "XL"
#     elif fit_score < 50:
...
#     # Fetch products from another API (if running separately)
#     try:
#         import requests
#         resp = requests.get("http://127.0.0.1:5001/products")
#         products = resp.json()
...
#     recommendations = [
#         p for p in products
#         if p["occasion"].lower() == occasion.lower() and suggested_size in p["sizes"]
#     ]
